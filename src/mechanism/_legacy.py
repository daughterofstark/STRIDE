"""
Lifted verbatim from v5 (final_code_piece.py).
M0 change: import-time argparse/execution removed; main loop wrapped in
run_pipeline(config); MDAnalysis import guarded so the package imports
without it (pure-stats CI). NO scientific logic changed.
"""
import os, sys, glob, subprocess, itertools, warnings, random, time
import numpy as np
import pandas as pd
import matplotlib
...
except Exception:  # keep package importable without MDAnalysis (M0)
    mda = None
    mda_align = None

# Pure numerical helpers now live in dedicated modules (single source of truth)
from .statistics.fdr import fdr_bh
from .statistics.correlation import pearson_both
from .statistics.convergence import block_conv_sem
from .statistics.xcorr import xcorr_fn
from .statistics.events import detect_near_closure_events
from .statistics.neff import attach_effective_sample_size  # [M1]
from .statistics.bootstrap import attach_bootstrap_ci  # [M2]
from .hierarchy.hierarchy import attach_structural_metadata  # [M3]
from .hierarchy.config_resolver import resolve_hierarchy_config  # [M3]
from .effects.labels import canon_label

# Config-derived globals (defaults; overwritten by run_pipeline)
TRIPLICATES_DIR  = os.path.expanduser('~/Desktop/actual_final_stuff_medha')
RUN_DIRS         = ['1st_run','2nd_run','3rd_run']
PROTEIN_WLIST    = None
SUBSAMPLE_STRIDE = 20
FRAME_PS         = 10.0
ANALYSIS_FRAME_PS= FRAME_PS * SUBSAMPLE_STRIDE
DO_SENSITIVITY   = False
FORCE_DYN_TRIAD  = False
SKIP_MSA         = False
GLOBAL_SEED      = 42
POVME_EXE        = None
HIERARCHY_CONFIG = None   # [M3] optional path to a hierarchy config (YAML/JSON)
_warned_msa      = set()
...
def run_pipeline(config):
    """Behaviour-preserving entry point. Sets config globals, then runs the
    verbatim v5 main loop. Identical outputs to v5 for the default config."""
    global TRIPLICATES_DIR, RUN_DIRS, PROTEIN_WLIST, SUBSAMPLE_STRIDE, FRAME_PS
    global ANALYSIS_FRAME_PS, DO_SENSITIVITY, FORCE_DYN_TRIAD, SKIP_MSA, GLOBAL_SEED, POVME_EXE
    global HIERARCHY_CONFIG
    TRIPLICATES_DIR   = os.path.abspath(config.base_dir)
    RUN_DIRS          = list(config.run_dirs)
    PROTEIN_WLIST     = config.proteins
    SUBSAMPLE_STRIDE  = config.stride
    FRAME_PS          = config.frame_ps
    ANALYSIS_FRAME_PS = FRAME_PS * SUBSAMPLE_STRIDE
    DO_SENSITIVITY    = config.sensitivity
    FORCE_DYN_TRIAD   = config.dynamic_triad
    SKIP_MSA          = config.no_msa
    GLOBAL_SEED       = config.seed
    HIERARCHY_CONFIG  = getattr(config, "hierarchy_config", None)
    ...
    for run_name in RUN_DIRS:
        ...
        for proj in candidates:
            ...
            # ───────────────────────────────────────────────────────────────
            # [M1] EFFECTIVE SAMPLE SIZE — uncertainty only.
            # Appends tau_int / n_eff / neff_status / theta_se using the
            # existing volumes and Cα distance matrix (read-only). Does NOT
            # touch the effect field (r/abs_r) or any existing column.
            # ───────────────────────────────────────────────────────────────
            df_res = attach_effective_sample_size(
                df_res, all_res, dm, volumes[:n_frames])

            # ───────────────────────────────────────────────────────────────
            # [M2] BLOCK-BOOTSTRAP CONFIDENCE INTERVALS — uncertainty only.
            # Reuses M1's tau_int (block length) and n_eff (fallback). Reads
            # volumes and the Cα distance matrix (read-only). Does NOT touch the
            # effect field or any existing column. Deterministic via the seed.
            # ───────────────────────────────────────────────────────────────
            df_res = attach_bootstrap_ci(
                df_res, all_res, dm, volumes[:n_frames], seed=GLOBAL_SEED)

            # ───────────────────────────────────────────────────────────────
            # [M3] STRUCTURAL METADATA — biological hierarchy (no inference).
            # Config-driven and family-agnostic. Reads canonical numbering via
            # the runtime offset; appends chain/domain/motif/secondary_structure/
            # region_id. Does NOT touch any existing column or any effect value.
            # ───────────────────────────────────────────────────────────────
            try:
                _hcfg = resolve_hierarchy_config(proj, HIERARCHY_CONFIG)
                df_res = attach_structural_metadata(
                    df_res, all_res, _hcfg, offset=offset)
            except Exception as _e:   # never break the pipeline on metadata
                print(f"  WARN [{tag}] structural metadata skipped: {_e}")
